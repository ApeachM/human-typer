...
- **Human-like 타이핑**: 실제 사람처럼 자연스러운 타이핑 시뮬레이션
  - **Exponential 분포**: 대부분 빠르게, 가끔 느리게 (uniform이 아닌 자연스러운 분포)
  - **문자별 딜레이**: 구두점/특수문자는 느리게, 일반 문자는 빠르게
  - **타이핑 리듬**: 빨라졌다 느려졌다 하는 자연스러운 속도 변화
  - **오타 시뮬레이션**: 가끔 오타 후 백스페이스로 수정 (연속 오타 지원)
  - **버스트 타이핑**: 단어 사이에 짧은 휴지
...
2. **대상 창 선택**:
   ...
   - (선택) "Fix" 버튼으로 대상 창 고정
3. **설정 조정**:
   - **Min Delay (ms)**: 키 입력 간 최소 딜레이 (기본값: 15ms)
   - **Max Delay (ms)**: 키 입력 간 최대 딜레이 (기본값: 70ms)
...
빠른 개발자 타이핑 속도(~100-150 WPM) 기준으로 설정됨:
- Min Delay: 15ms
- Max Delay: 70ms

실제 딜레이는 다음 요소들에 의해 조정됩니다:
- **문자 타입**: 구두점(1.8x), 특수문자(2.0x), 대문자(1.4x), 엔터(4.0x)
- **타이핑 리듬**: 0.6x ~ 1.8x 범위에서 자연스럽게 변동
- **오타 확률**: 2% (연속 오타 시 최대 37%까지 증가)
